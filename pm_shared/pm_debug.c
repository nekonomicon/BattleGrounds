...
int gHullsFixed = 0;
const vec3_t PM_VEC_HULL_MIN = {-16, -16, -36};
const vec3_t PM_VEC_HULL_MAX = { 16,  16,  36};
const vec3_t PM_VEC_DUCK_HULL_MIN = {-16, -16, -18};
const vec3_t PM_VEC_DUCK_HULL_MAX = { 16,  16,  36};
const vec3_t PM_POINT_MIN = { 0, 0, 0 };
const vec3_t PM_POINT_MAX = { 0, 0, 0 };

void PM_UpdateClipBox( void )
{
    VectorCopy(PM_POINT_MIN, pmove->player_mins[2]);
    VectorCopy(PM_POINT_MAX, pmove->player_maxs[2]);
	VectorCopy(PM_VEC_HULL_MIN, pmove->player_mins[0]);
	VectorCopy(PM_VEC_HULL_MAX, pmove->player_maxs[0]);
	VectorCopy(PM_VEC_DUCK_HULL_MIN, pmove->player_mins[1]);
	VectorCopy(PM_VEC_DUCK_HULL_MAX, pmove->player_maxs[1]);
}
...
{ 
	...
	// world hull 0 is point, while model hull 2 is point
	VectorCopy(PM_POINT_MIN, pmove->physents[0].model->hulls[0].clip_mins);  
	VectorCopy(PM_POINT_MAX, pmove->physents[0].model->hulls[0].clip_maxs);
	VectorCopy(PM_VEC_HULL_MIN, pmove->physents[0].model->hulls[1].clip_mins);
	VectorCopy(PM_VEC_HULL_MAX, pmove->physents[0].model->hulls[1].clip_maxs);
	VectorCopy(PM_VEC_DUCK_HULL_MIN, pmove->physents[0].model->hulls[3].clip_mins);
	VectorCopy(PM_VEC_DUCK_HULL_MAX, pmove->physents[0].model->hulls[3].clip_maxs);
	...
	for (model = 0; model < 1024; model++) {  
		if (g_modelarray[model].name[0] == '*')   {
			VectorCopy(PM_POINT_MIN, g_modelarray[model].hulls[0].clip_mins);  
			VectorCopy(PM_POINT_MAX, g_modelarray[model].hulls[0].clip_maxs);
			VectorCopy(PM_VEC_HULL_MIN, g_modelarray[model].hulls[1].clip_mins);
			VectorCopy(PM_VEC_HULL_MAX, g_modelarray[model].hulls[1].clip_maxs);
			VectorCopy(PM_VEC_DUCK_HULL_MIN, g_modelarray[model].hulls[3].clip_mins);
			VectorCopy(PM_VEC_DUCK_HULL_MAX, g_modelarray[model].hulls[3].clip_maxs);		
		}
	}
}
