...
Vector CBaseEntity::FireBulletsPlayer ( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker, entvars_t *pevInflictor, int shared_rand )
{
	...
	// laser tag musket beams
#ifdef CLIENT_DLL
	extern int g_iLaserTag;
	if(g_iLaserTag == 1)
#else
	if(CVAR_GET_FLOAT("mp_lasertag") == 1.0)
#endif
	{
		MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
			...
			WRITE_BYTE( 20 );  // width
			WRITE_BYTE( 0 );   // noise
			if (pevAttacker->team == BRITS)
			{
				WRITE_BYTE( 255 );   // r, g, b
				WRITE_BYTE( 16 );   // r, g, b
				WRITE_BYTE( 16 );   // r, g, b
			}
			if (pevAttacker->team == AMERICANS)
			{
				WRITE_BYTE( 16 );   // r, g, b
				WRITE_BYTE( 16 );   // r, g, b
				WRITE_BYTE( 255 );   // r, g, b
...
float CBaseEntity::GetDamage(entvars_t *pevAttacker, Vector vecDir, TraceResult *ptr, float BulletTravelDistance)
{
	...
	GetClassPtr((CBasePlayer*)pevAttacker)->m_pActiveItem->m_flSkillModifier;
	...
	// different behaviour for rifles
	CBasePlayer *Attacker = GetClassPtr((CBasePlayer*)pevAttacker);//(VARS(pAttackingPlr));
	bool rifle = false;
	if (pAttacker->IsPlayer())
		if((!strcmp(Attacker->m_szClassName, "Loyalist")) || (!strcmp(Attacker->m_szClassName, "Minuteman")))
			rifle = true;
	...
	if(BulletTravelDistance >= 800.0) 
	{
		if(BulletTravelDistance <= 3000) 
		{
			if (rifle)
			{
				// ramp between 1 and 0.7 between 800 and 3000 units
				float flDistanceModifier = ((8133 - BulletTravelDistance) / 7332);
				flDamage *= flDistanceModifier;
			}
			else
			{
				// ramp between 1 and 0.3 between 800 and 3000 units
				float flDistanceModifier = ((3942 - BulletTravelDistance) / 3140);
				flDamage *= flDistanceModifier;
			}
			
		// use  * 0.3 and  * 0.6 for rifles damage at 3000 and over
		} 
		else 
		{
			if (rifle)
