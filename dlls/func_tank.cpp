...
//		direction = m_sightOrigin - barrelEnd;
	...
	if (gpGlobals->time > m_NextCannonHudUpdate)
	{
		...
		// to fix bug of no cannonball being spawned when shooting straight ahead
		if (angles[1] == 270)
			angles[1] = 269;
		if (angles[1] == 0)
			angles[1] = 1;
	...
	if ( m_fireLast != 0 )
	{
		...
		if ( bulletCount > 0 )
		{
			...
			MESSAGE_BEGIN(MSG_ALL, gmsgParticles);
				...
				WRITE_COORD( forward.x );
				WRITE_COORD( forward.y );
				WRITE_COORD( forward.z );
				WRITE_SHORT(iDefaultCannonSmoke);
			MESSAGE_END(); 
