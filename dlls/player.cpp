...
//	gmsgArmLegShot = REG_USER_MSG("ArmLegShot", 1);//Possibly here aswell lol Myristate
	...
	// BP LaserTag
	gmsgLaserTag = REG_USER_MSG("LaserTag", 1);
...
void CBasePlayer::PlayerDeathThink(void)
{
	...
	if(!IsObserver() && m_bHasDied == false)
	{
		...
		StartObserver(pev->origin, pev->v_angle);
		...
		for(int i = 0; i <= gpGlobals->maxClients; i++)
		{
			...
			if(pPlayer && pPlayer->pev->iuser1 != 0 && pPlayer->m_hObserverTarget != NULL && pPlayer->m_hObserverTarget->pev == pev && pPlayer != this)
			{
				pPlayer->m_hObserverTarget = m_hObserverTarget;
				pPlayer->pev->iuser2 = ENTINDEX( m_hObserverTarget->edict() );
...
bool IsSpawnPointValid( CBaseEntity *pPlayer, CBaseEntity *pSpot )
{
	...
	while ( (ent = (CBaseEntity*)UTIL_FindEntityInSphere( ent, pSpot->pev->origin, 64 )) != NULL )
	{
	...
	for(int i = 0; i <= gpGlobals->maxClients; i++)
	{
		...
		if(player && pPlayer->edict() != player->edict())
		{
			Vector *vecOrigin = pOriginTracker->GetPlayerOrigin(i);
			float flLength = (*vecOrigin - pSpot->pev->origin).Length();

			if(flLength >= -32.0 && flLength <= 32.0)
			return false;
		}
	}
	return true;
...
edict_t *EntSelectTeamSpawnPoint( CBasePlayer *pPlayer )
{
    ...
    if ( FNullEnt( pSpot ) || pSpot->pev->iuser1 == 0) // skip over the null point and points which are inactive
    {			
    ...
    do
    {
        if ( pSpot )
        {
            ...
            if ( pSpot->pev->iuser1 == 1 && IsSpawnPointValid( pPlayer, pSpot )  )
            {
    ...
    if ( !FNullEnt( pSpot ) && pSpot->pev->iuser1 == 1 )
    {
        CBaseEntity *ent = NULL;
...
//	ResetClock();//Baer
	...
	pev->view_ofs = Vector(0, 0, VEC_VIEW);
	Precache();
...
void CBasePlayer::ResetSpeed()
{
	if(g_pBGRules->pRoundRules->GetRoundState() == ROUND_PAUSED)
		...
		float flSpeed = iClassSpeeds[pev->team][pev->playerclass];
...
void CBasePlayer::SetObjectCollisionBox( void ) {
	if(FBitSet(pev->flags, FL_DUCKING)) {
		pev->absmin = pev->origin + VEC_DUCK_HULL_MIN;
		pev->absmax = pev->origin + VEC_DUCK_HULL_MAX;
	} else {
		pev->absmin = pev->origin + VEC_HULL_MIN;
		pev->absmax = pev->origin + VEC_HULL_MAX;
	}
}
//Ben
